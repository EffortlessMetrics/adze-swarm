...
fn assert_bad_input_diagnostic(diagnostic: &serde_json::Value) {
    ...
    assert!(
        ...
        "diagnostics projection should name the invalid source token: {diagnostic:?}"
    );
    assert!(
        diagnostic["message"]
            .as_str()
            .is_some_and(|message| !message.starts_with("end;")),
        "diagnostics projection should not report non-EOF invalid input as EOF: {diagnostic:?}"
    );
}
