...
pub struct ParseStack {
    ...
    /// Stack of subtrees
    pub(super) nodes: Vec<Arc<Subtree>>,
...
impl ParseStack {
    pub(super) fn new(initial_state: StateId, id: usize) -> Self {
        Self {
    ...
    pub(super) fn current_state(&self) -> StateId {
        self.states.last().copied().unwrap_or(StateId(0))
    }

    /// Push a new state and node
    pub(super) fn push(&mut self, state: StateId, node: Arc<Subtree>) {
    ...
    pub(super) fn pop(&mut self, n: usize) -> Vec<Arc<Subtree>> {
    ...
    pub(super) fn fork(&self, new_id: usize) -> Self {
        Self {
    ...
    #[allow(dead_code)]
    pub(super) fn print_tree_structure(node: &Arc<Subtree>, indent: usize) {
    ...
    #[allow(dead_code)]
    pub(super) fn has_equivalent_parse_tree(&self, other: &ParseStack) -> bool {
    ...
    #[allow(dead_code)]
    pub(super) fn nodes_structurally_equivalent(
        node1: &Arc<Subtree>,
        node2: &Arc<Subtree>,
    ) -> bool {
