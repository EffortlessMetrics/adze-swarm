...
mod tests {
    ...
    fn test_parse_bytes_with_trailing_nul_appends_sentinel() {
        ...
        // own EOF sentinel. This exercises the real `Parser::parse_bytes`
        // entry point end-to-end (not just the `append_sentinel` helper in
        // isolation) to prove the invariant holds through the full call
        // path every EOF check in `lex_token` depends on.
        //
        // The language below mirrors the minimal arithmetic-expression
        // table used by `runtime/tests/test_pure_parser.rs::create_test_language`,
        // which is exercised end-to-end (including `lex_fn: None` fallback
        // lexing) by that file's existing passing tests. Null/undersized
        // tables here would make the real parse loop dereference garbage,
        // so this keeps every table non-null and consistently sized rather
        // than trimming it down to the bare minimum.
        ...
        parser
            ...
            .expect("minimal language should validate");

        let source: &[u8] = b"a\0";
        parser.parse_bytes(source);

        let lexer_input_len = parser
            .lexer
            .as_ref()
            .expect("parse_bytes initializes the lexer")
            .input
            .len();
        assert_eq!(
            lexer_input_len,
            source.len() + 1,
            "a real trailing NUL byte in `source` must not be mistaken for the lexer's EOF sentinel"
        );
    }
