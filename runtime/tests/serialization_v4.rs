...
//! Display formatting, equality, cloning, parse_sexpr, and edge cases.
...
#[test]
fn test_parse_sexpr_parses_list_items() {
    let result = parse_sexpr("(+ 1 2)").unwrap();
    let items = result.as_list().unwrap();
    assert_eq!(items.len(), 3);
    assert_eq!(items[0].as_atom(), Some("+"));
    assert_eq!(items[1].as_atom(), Some("1"));
    assert_eq!(items[2].as_atom(), Some("2"));
}
...
fn test_parse_sexpr_nested_input() {
    ...
    assert_eq!(
        result,
        Ok(SExpr::List(vec![SExpr::List(vec![SExpr::Atom(
            "nested".to_string()
        )])]))
    );
}
