...
CANARIES=(
  ...
  "Product Proof stable surface routing|cargo test -p adze-cli product_proof_workflow_routes_stable_claim_surfaces -- --exact --nocapture"
  ...
  "co-release dependency claim boundary|cargo test -p adze-cli co_release_dependency_snippets_stay_release_surface_bounded -- --exact --nocapture"
  ...
  'standalone downstream starter binary run|cargo run --manifest-path testing/downstream-starter/Cargo.toml --example parse -- "1 + 2 * 3"'
