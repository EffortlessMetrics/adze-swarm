...
if rg -n '^[[:space:]]*runs-on:[[:space:]]*self-hosted[[:space:]]*$' "$workflow_dir"; then
  echo "Bare scalar self-hosted runs-on is forbidden." >&2
  bad=1
fi

while IFS=: read -r file line _; do
  window="$(sed -n "${line},$((line+18))p" "$file")"
...
while IFS=: read -r file line _; do
  ...
  if ! printf '%s\n' "$window" | rg -q 'group:[[:space:]]*em-ci-' ||
     ! printf '%s\n' "$window" | rg -q '(em-ci|ci-nano|policy-nano|workflow-nano|rust-tiny|rust-medium|rust-large|rust-16gb|cx23|cx33|cx43|cx53|cpx42)'; then
    echo "$file:$line: self-hosted mapping lacks explicit em-ci group/capacity labels" >&2
    bad=1
  fi
done < <(rg -n '^[[:space:]]*runs-on:[[:space:]]*$' "$workflow_dir" || true)
