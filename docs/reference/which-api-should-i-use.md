...
| Tree-sitter-style tree traversal | `document.as_tree_sitter()` or `adze::ts_compat::Tree::from_document(...)` | Adapts document facts into the selected-tree compatibility subset. | Not full Tree-sitter runtime, query, node-types, or imported-corpus parity. |
