...
| Pure-Rust generated parsers can be used from clean downstream crates. | Stable | `SUPPORT_TIERS.md`; README quickstart; getting-started tutorial; adze-swarm#617 | `just ci-product-stable`; `cargo test -p adze-cli readme_arithmetic_quickstart_builds_and_runs -- --exact --nocapture`; `cargo test -p downstream-demo -- --nocapture`; `cargo run -p downstream-demo --quiet`; `cargo test --manifest-path testing/downstream-starter/Cargo.toml`; `cargo run --manifest-path testing/downstream-starter/Cargo.toml --example parse`; #617 receipt: checkout-built `adze init calc`, generated `cargo test`, generated `cargo run --example parse -- "1 + 2 * 3"`, and `cargo test -p adze-cli release_surface_bounded -- --nocapture` on `24650183` | Stable claim with downstream clean-room and checkout-built first-use proof. No crates.io install claim. |
