...
| Runtime S-expression helpers can emit parseable canonical `SExpr` strings. | Stabilizing | `SUPPORT_TIERS.md`; `docs/reference/api.md`; #706/#708 receipts | `cargo test -p adze --features serialization --test test_serialization_roundtrip sexpr -- --nocapture`; `cargo test -p adze --features serialization --test tree_sexp_serialization_comprehensive sexp -- --nocapture`; `cargo test -p adze --features serialization parse_sexpr -- --nocapture` | Non-Stable helper claim under the `serialization` feature. `SExpr::to_canonical_sexpr()` / `write_canonical_sexpr()` are parseable by `parse_sexpr`, `Display for SExpr` remains raw/debug-style, and no CLI/WASM schema, release, public install, or support-tier promotion claim is created. |
